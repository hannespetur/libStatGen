...
void
PhoneHome::enableCompletionStatus(const char * programName)
{
  if (programName != NULL)
  {
    add("pgm", programName);
...
void
PhoneHome::disableCompletionStatus()
{
...
bool
PhoneHome::checkVersion(const char * programName, const char * version,
                        const char * params)
{
...
void
PhoneHome::completionStatus(const char * status, const char * programName)
{
  if (programName != NULL)
  {
    add("pgm", programName);
...
void
PhoneHome::add(const char * name, const char * val)
{
...
PhoneHome::connect()
{
  ...
  knet_silent(1);
  knetFile * file = knet_open2(ourURL.c_str(), "r");
  if (file == 0)
    return false;
  ...
  {
    numRead = knet_read2(file, buf, readLen);
    buf[numRead] = '\0';
    ourReturnString += buf;
  }
